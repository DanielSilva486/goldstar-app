...
// CORREÇÃO DO BOTÃO FECHAR AQUI (A palavra "fechar")
export default function ModalConfiguracoes({ fechar }) {
  ...
  const confirmarAcao = (texto, acao) => setModalConfirmacao({ visivel: true, texto, acao });
  ...
  const carregarDados = () => {
    ...
    // Agora a configuração busca TODOS (ativos e inativos)
  ...
  // --- NOVA FUNÇÃO: DESATIVAR / REATIVAR FUNCIONÁRIO ---
  const alternarStatus = (id, statusAtual) => {
    const acao = statusAtual === false ? "reativar" : "bloquear";
    confirmarAcao(`Tem certeza que deseja ${acao} esta profissional?`, async () => {
      fecharConfirmacao();
      await fetch(`https://goldstar-backend-9m2p.onrender.com/api/colaboradores/${id}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ativo: statusAtual === false ? true : false })
      });
      mostrarAlerta(`Profissional atualizada!`, 'sucesso');
      carregarDados();
    });
  };
  ...
  const mudarAba = (aba) => { setAbaAtiva(aba); setTermoBusca(''); };
    ...
    <div className="absolute inset-0 bg-gray-900/60 flex justify-center items-center z-50 p-4">
      <div className="bg-white w-full max-w-lg rounded-3xl p-6 flex flex-col shadow-2xl relative max-h-[90vh] overflow-hidden">
        ...
        {modalConfirmacao.visivel && (
          <div className="absolute inset-0 bg-white/90 backdrop-blur-sm flex flex-col justify-center items-center z-50 p-6 text-center">
             ...
             <div className="flex gap-3">
               ...
               <button onClick={modalConfirmacao.acao} className="px-5 py-2 rounded-xl font-bold text-white bg-red-500 hover:bg-red-600 transition-colors">Confirmar</button>
             </div>
          </div>
        )}

        {/* CORREÇÃO AQUI: onClick chama fechar */}
        ...
        <div className="mb-4 relative">
          ...
          <input type="text" placeholder={`Buscar ${abaAtiva === 'servicos' ? 'serviço' : 'profissional'}...`} value={termoBusca} onChange={(e) => setTermoBusca(e.target.value)} className="w-full pl-10 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 transition-all"/>
        </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {colabFiltrados.map(c => (
                  <div key={c.id} className={`flex justify-between items-center p-3 border rounded-xl shadow-sm transition-colors ${c.ativo === false ? 'bg-gray-50 border-gray-200' : 'bg-white border-gray-100 hover:border-gray-300'}`}>
                    
                    <span className={`text-sm font-medium ${c.ativo === false ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                      {c.nome} 
                      <span className={`font-bold ml-2 no-underline ${c.ativo === false ? 'text-gray-400' : 'text-orange-500'}`}>{Number(c.percentual_comissao)}%</span>
                      {c.ativo === false && <span className="ml-2 text-xs text-red-500 font-bold no-underline">(Bloqueada)</span>}
                    </span>

                    <button 
                      onClick={() => alternarStatus(c.id, c.ativo)} 
                      title={c.ativo === false ? "Reativar Profissional" : "Bloquear Profissional"}
                      className={`p-1.5 rounded-md transition-colors ${c.ativo === false ? 'text-green-500 hover:bg-green-50' : 'text-red-400 hover:text-red-600 hover:bg-red-50'}`}
                    >
                      {c.ativo === false ? (
